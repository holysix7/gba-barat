...
class PendaftaranController extends Controller
{
    public function index(){
        $maxDate = date('Y-m-d');
        return view('autodebit.mygoal.pendaftaran.index', compact('maxDate'));
    }
        ...
        if($request->start_date){
            $records    = DB::table('savdep_product_customer_mygoals AS a')
                ...
                ->where(function($query){
                    $query->where('sp_pc_status', 1)
                    ->orWhere('sp_pc_status', 5)
                    ->orWhere('sp_pc_status', 9);
                })
            ...
            $records    = DB::table('savdep_product_customer_mygoals AS a')
                ...
                ->where(function($query){
                    ...
                    ->orWhere('a.sp_pc_status', 5)
                    ->orWhere('a.sp_pc_status', 9);
                })
                ->orderBy('a.sp_pc_reg_date', 'ASC');
        }
    ...
    public function export(Request $request){
        $date = date("Ymd-his");
        userActivities('Export', 'Melakukan export data laporan pendaftaran', 'savdep_product_customer_mygoals', 'General', Route::current()->getName());
        return Excel::download(new PendaftaranExport($request), 'laporan-pendaftaran-'. $date .'.xlsx');
    }
}