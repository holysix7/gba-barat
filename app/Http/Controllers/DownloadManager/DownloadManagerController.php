...
class DownloadManagerController extends Controller
{
  public function index(){
    return view('download-manager.index');
  }
  ...
  public function ajax(Request $request){
    ...
    if($request->produk_id != 'ALL'){
      $sql = $sql->where('document_type', 'ilike', "%$request->produk_id%");
    }
    if($request->kategori_id != 'ALL'){
      $sql = $sql->where(function($query) use($request){
        $query->where('filename', 'ilike', "%$request->kategori_id%");
      });
    }
    ...
    foreach($records as $row){
      $row->rownum = $i++;
      $row->route = route('download-manager.download', Crypt::encrypt($row->path . $row->filename));
    }
    $response = [
