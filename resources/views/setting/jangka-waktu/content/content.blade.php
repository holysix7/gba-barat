...
<script type="text/javascript">
        ...
        },
        {
          ...
          mRender: function(data, type, row) {
            ...
            if($('#updatePermission').val() == 'true'){
              html = `<a href="javascript:void(0)" class="button-action" style='font-size: 28px;' onclick="modalFunction('${row.sd_pp_code}', 'edit')"><i class="mdi mdi-tooltip-edit"></i></a>`
            }
            if($('#deletePermission').val() == 'true'){
              html  += `<a href="javascript:void(0)" class="button-action" style='font-size: 28px;' onclick="modalFunction('${row.sd_pp_code}', 'delete')"><i class="mdi mdi-delete"></i></a>`
            }
            return html
          }
        },
  ...
  function modalFunction(sd_pp_code, type){
    $('#infoModal').modal('show')
    var title = 'Delete Data'
    var route = '{{ route("setting.jangkawaktu.delete") }}'
    if(type == 'edit'){
      title = 'Edit Data'
      route = '{{ route("setting.jangkawaktu.update") }}'
    }
    ...
    $.ajax({
      url: "{{ route('setting.jangkawaktu.show') }}",
      type: 'POST',
      data: {
        _token: $('meta[name="csrf-token"]').attr('content'),
        sd_pp_code: sd_pp_code
      },
      success: function(response){
        ...
        $('#input1').html(`<input type="text" class="form-control" name="sd_pp_code" value="${response.sd_pp_code}" readonly>`)
        $('#label2').html(`<label class="mt-2">Nama</label>`)
        $('#label2').html(`<label class="mt-2">Deskripsi</label>`)
        $('#input2').html(`<input type="text" class="form-control" name="sd_pp_desc" value="${response.sd_pp_desc}" readonly>`)
        if(type == 'edit'){
          $('#input2').html(`<input type="text" class="form-control" name="sd_pp_desc" value="${response.sd_pp_desc}">`)
        }
      }
    })
  }
