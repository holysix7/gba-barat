  ...
  <div class="container-fluid">
    ...
    @if(empty(request()->segment(4)))
      <div class="row header-menu">
        <div class="col-md-12">
          <form action="{{ route('autodebit.regular.daftar-rekening.export') }}" enctype="multipart/form-data" method="POST">
            @csrf
            <div class="row">
              <div class="col-sm-9">
                <div class="row">
    ...
    <div class="card card-primary card-outline card-outline-tabs col-sm-12">
      <div class="row">
        <div class="card-body col-12">
          @if(empty(request()->segment(4)))
            @include('autodebit.regular.daftar-rekening.content.content')
          @else
            @if(request()->segment(4) == 'new')
              @include('autodebit.regular.daftar-rekening.content.new')
            @elseif(request()->segment(4) == 'confirm')
              @include('autodebit.regular.daftar-rekening.content.confirm')
            @elseif(request()->segment(4) == 'result')
              @include('autodebit.regular.daftar-rekening.content.result')
            @else
              @include('autodebit.regular.daftar-rekening.content.show')
            @endif
          @endif
        </div>
      </div>
